...
class FaceModelGenerator:
    def __init__(self, video_path):
        ...
        # Cap at 30 FPS or video FPS if lower
        self.target_fps = min(30, self.video_fps)
        self.frame_count = 0
        self.process_resolution = (self.video_width, self.video_height)
        self.display_resolution = (self.video_width, self.video_height)
                ...
                for event in pygame.event.get():
                    if event.type == pygame.QUIT:
                        ...
                        if event.key == pygame.K_s:
                            ...
                            # Seek backward 5 frames
                            current_frame = self.video_capture.get(
                                cv2.CAP_PROP_POS_FRAMES)
                            self.video_capture.set(
                                cv2.CAP_PROP_POS_FRAMES, max(0, current_frame - 5))
                        elif event.key == pygame.K_RIGHT:
                            # Seek forward 5 frames
                            self.video_capture.set(cv2.CAP_PROP_POS_FRAMES,
                                                   self.video_capture.get(cv2.CAP_PROP_POS_FRAMES) + 5)
                    elif event.type == pygame.MOUSEMOTION:
                        if event.buttons[0]:  # Left mouse button
                            self.rotation[1] += event.rel[0]
                            self.rotation[0] += event.rel[1]

                # Process video frame if not paused
                if not self.paused:
                    ...
                    # Draw video feed
                    glViewport(0, 0, self.display_width //
...
if __name__ == "__main__":
    video_path = "video.mov" # Path to video file - change this
    generator = FaceModelGenerator(video_path)
    generator.run()
